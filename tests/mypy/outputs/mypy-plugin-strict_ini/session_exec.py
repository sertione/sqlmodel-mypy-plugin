...
from sqlalchemy.engine.result import Result, TupleResult
...
with Session(engine) as session:
    ...
    # accept SQLAlchemy `select(...)` statements (commonly used with SQLModel)
    select_result = session.exec(select(Hero))
    _ok_select_result: TupleResult[tuple[Hero]] = select_result
# MYPY: error: Incompatible types in assignment (expression has type "CursorResult[Any]", variable has type "TupleResult[tuple[Hero]]")  [assignment]
...
async def async_exec(session: AsyncSession) -> None:
    ...
    select_result2 = await session.exec(select(Hero))
    _ok_select_result2: TupleResult[tuple[Hero]] = select_result2
# MYPY: error: Incompatible types in assignment (expression has type "CursorResult[Any]", variable has type "TupleResult[tuple[Hero]]")  [assignment]
